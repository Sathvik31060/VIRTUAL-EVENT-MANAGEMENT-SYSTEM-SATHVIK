...
const SessionDetail = () => {
    ...
    const sessionDetails = {
        1: { 
            ...
            videoLink: 'https://www.youtube.com/watch?v=wUHBoNOmGzs&pp=ygUTV2ViaW5hciBvbiBBSSBUb29scw%3D%3D' // Example video link
        },
        2: { 
            title: 'Work Shop', 
            date: '2024-10-02', 
            ...
            videoLink: 'https://www.youtube.com/watch?v=4-RZRLdBpFc&t=26s' // Example video link
        },
    };

    const session = sessionDetails[id];

    // Convert YouTube links to embeddable format with optional autoplay parameter
    const getEmbedLink = (url) => {
        if (!url) return '';
        let embedUrl = url.replace('watch?v=', 'embed/').split('&')[0];
        // Adding autoplay parameter if needed
        embedUrl += '?autoplay=1';
        return embedUrl;
    };
            ...
            {showVideo && (
                <div className="video-container">
                    <iframe
                        width="560"
                        height="315"
                        src={getEmbedLink(session.videoLink)}
                        title="Session Video"
                        frameBorder="0"
                        allowFullScreen
                    ></iframe>
